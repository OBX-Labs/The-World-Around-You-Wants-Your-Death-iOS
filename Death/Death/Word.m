...
- (void) build:(OKWordObject*)aWordObj renderingBounds:(CGRect)aRenderingBounds
{
    value = [[NSString alloc] initWithString:aWordObj.word];
     ...
     NSLog(@"Accuracy: %d", WORD_ACCURRACY);
...
//Build up the words based on png image font.
- (id) initWithWordandImageFont:(OKWordObject*)aWordObj withFont:(Font *)theFont renderingBounds:(CGRect)aRenderingBounds
{
    ...
    if(self)
    {
        ...
        currentWord =[[NSString alloc] initWithString:[aWordObj word]];
        
        //assign the visible bool for each letters of the word
        visibleLetters = [[NSMutableArray alloc] init];
        for(int i=0; i<[currentWord length];i++)
        {
            [visibleLetters addObject:[NSNumber numberWithBool:YES]];
        }
...
- (void) drawWithImage
{
    ...
    if(currentWord!=nil)
    {
        ...
        [defaultFont drawStringAt:currentPosition withZValue:0 andYRot:0 text:currentWord visible:visibleLetters isFocused:NO];
        //[defaultFont drawStringAt:currentPosition withZValue:0 andYRot:0 text:@"TEST" isFocused:NO];
    }
...
-(void) decontract {
    ...
    for(TessGlyph *aTessGlyph in glyphs ){
        ...
        //reload original data before contract
        //[aTessGlyph buildWithAccuracy:4];
    }
...
-(void) flicker:(int)index s:(float)aSpeed t:(float)time a:(float)activity
{
    ...
    float asdf = (activity+0.01)*30;
   // NSLog(@"Actvity=%f", asdf);
    for(int i=0;i<[visibleLetters count];i++)
    {
        //Use of OKNoise is very slow
        //bool visible =([OKNoise noiseX:index/10 y:[self getMillis]/10000]> activity) && (fabs([OKNoise noiseX:index*100 y:[self getMillis]*speed]-0.5) > time/2);
        //bool visible = arc4random()%100 > asdf;
        
        bool visible;
        if([[visibleLetters objectAtIndex:i] boolValue])
        {
            visible=arc4random()%100 > asdf;
        }
        else
        {
            visible=arc4random()%100 <10;
        }
        [visibleLetters replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:visible]];
