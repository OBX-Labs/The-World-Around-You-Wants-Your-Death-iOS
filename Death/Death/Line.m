...
//- (id) initWithScale:(float)aScale font:(OKTessFont*)aFont imageFont:(Font*)imageFont source:(NSArray*)aSource start:(int)aStart renderingBounds:(CGRect)aRenderingBounds positionY:(float)positionY
{
    self = [super init];
    if(self)
    {
        ...
        {
            Word *word = [[Word alloc] initWithWord:[aSource objectAtIndex:i] font:aFont renderingBounds:aRenderingBounds];
            //Word *word =[[Word alloc] initWithWordString:[[aSource objectAtIndex:i] word] withFont:imageFont renderingBounds:aRenderingBounds];
...
//- (id) initWithScale:(float)aScale font:(OKTessFont*)aFont imageFont:(Font*)imageFont source:(NSArray*)aSource start:(int)aStart renderingBounds:(CGRect)aRenderingBounds positionY:(float)positionY
{
    ...
    if(self)
    {
        ...
        for(int i = 0; i < [aSource count]; i++)
        {
            ...
            //Word *word = [[Word alloc] initWithWord:[aSource objectAtIndex:i] font:aFont renderingBounds:aRenderingBounds];
            Word *word =[[Word alloc] initWithWordandImageFont:[aSource objectAtIndex:i] withFont:imageFont renderingBounds:aRenderingBounds];
            
            //posX += ([word getSize].width/2)*aScale;
            [word setPosX:posX y:pos.y z:0];
            [word setRealPosX:posX y:positionY];
            //posX += ([word getSize].width/2)*aScale + SPACE_WIDTH*aScale
            posX += ([word getSize].width)*aScale + SPACE_WIDTH*aScale;
...
/*
//create a list of the words for png font rendering
- (id)initWithSentence:(NSString *)theSentence withFont:(Font *)theFont scale:(float)aScale start:(int)aStart renderingBounds:(CGRect)aRenderingBounds positionY:(float)positionY
{
	self = [self init];
	if (self != nil)
	{
		...
		{
            currentWord = [[Word alloc] initWithWordImage:[theWords objectAtIndex:i] isAutoplay:NO withFont:theFont renderingBounds:rBounds];
...
-(void) flicker:(int)index s:(float)aSpeed t:(float)time a:(float)activity
{
    ...
    for(Word *aWord in words)
        [aWord flicker:index s:aSpeed t:time a:activity];
}
