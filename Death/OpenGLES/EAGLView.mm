...
@synthesize animating, animationFrameInterval, displayLink, animationTimer, death;
...
- (void) setup
{
    ...
    NSLog(@"TEXTPATH=%@", textPath);
    NSMutableString *funnelTextFile = [NSMutableString stringWithContentsOfFile:textPath encoding:NSUTF8StringEncoding error:nil];

     NSLog(@"funnelTextFile=%@", funnelTextFile);
    
    //load the second text direct from file
    NSString* pathRight = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"right"] ofType:@"txt"];
     NSLog(@"RIGHT PATH=%@", pathRight);
    NSMutableString* funnelTextFileRight = [NSMutableString stringWithContentsOfFile:pathRight encoding:NSUTF8StringEncoding error:nil];
    
    NSLog(@"Funnel Text Right=%@", funnelTextFileRight);
    
    // Fonts
    NSString *fontName = [OKPoEMMProperties objectForKey:FunnelFont];
    
    // Clean text
    //replace em dash (charID 8212) with - (charID 45)
    unichar emdash = 8212;
    [funnelTextFile replaceOccurrencesOfString:[NSString stringWithFormat:@"%C", emdash] withString:@"-" options:NSCaseInsensitiveSearch range:NSMakeRange(0, [funnelTextFile length])];
    ...
    //get the texts for left and right
    //funnelText = [[OKTextObject alloc] initWithText:funnelTextFile withTessFont:font andCanvasSize:sBounds.size];
    OKTextObject *funnelText = [[OKTextObject alloc] initWithText:funnelTextFile withTessFont:font andCanvasSize:self.frame.size];
    OKTextObject *funnelTextRight = [[OKTextObject alloc] initWithText:funnelTextFileRight withTessFont:font andCanvasSize:self.frame.size];

    
    if(death) {
        //if it's an update after text edit, update both sides with same text
        [death updateFunnelText:funnelText];
    }
    else{
        //new execution: separate left and right
        death= [[Death alloc]initWithFont:font text:funnelText andBounds:(CGRect)self.frame eaglview:self];
        [death updateFunnelTextLeft:funnelText right:funnelTextRight];
    }
    ...
    for(UITouch *touch in touches)
    {
        ...
        if(fID == -1)
        {
            ...
            if(nfID != -1)
            {
                ...
                [death touchesBegan:nfID atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
            }
        }
    }
...
{    
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [death touchesMoved:fID atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
        }
    }
}
...
{   
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [death touchesEnded:fID atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
...
{    
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [death touchesCancelled:fID atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
