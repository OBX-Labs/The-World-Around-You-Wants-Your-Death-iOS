...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // TestFlight
    //[TestFlight takeOff:@"a059f578-bf42-4a0c-ba43-152f26dc9014"];
        ...
        if (![[OKTextManager sharedInstance] loadTextFromPackage:textKey atIndex:0])
        {
            ...
            if(![[OKTextManager sharedInstance] loadCustomTextFromPackage:textKey])
            {
                if(![[OKTextManager sharedInstance] loadTextFromPackage:defaultTextKey atIndex:0])
                {
                    ...
                    // Load new
                    if(![[OKTextManager sharedInstance] loadTextFromPackage:@"net.obxlabs.Death.jlewis.Death" atIndex:0])
                    {
                        // Epic fail
...
#warning SM : put the good App ID here
    [Appirater setAppId:@"778145716"];
