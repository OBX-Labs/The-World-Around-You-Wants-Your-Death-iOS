...
/* Return a OKPoint normalized to magnitude of 1*/
 
CG_INLINE OKPoint OKPointNormalize (OKPoint point1);
...
CG_INLINE OKPoint
OKPointNormalize (OKPoint point1)
{
    //normalize to vector 1, on x and y
    float magn = OKPointMag(point1);
    float newX = point1.x/magn;
    float newY = point1.y/magn;
    return(OKPointMake(newX, newY, 0.0f));
}
