    ...
    if(![[NSFileManager defaultManager] fileExistsAtPath:listPath])
    {
        ...
        for(NSString *packageKey in packageKeys)
        {
            ...
            for(NSDictionary *textDict in package)
            {
                ...
                NSString *fontFile2 = [properties objectForKey:FontFile2];
                ...
                if(fontFile2)
                {
                    [OKTextManager copyFile:fontFile2 ofType:@"fnt" fromBundleTo:fontsPath];
                    [OKTextManager copyFile:fontFile2 ofType:@"png" fromBundleTo:fontsPath];
                }
                else
                {
                    NSLog(@"Error: init - font file 2 is empty.");
                }
...
- (BOOL) saveCustomText:(NSString*)text forTitle:(NSString*)title
{
    ...
    NSArray *lines = [withoutWhiteSpace componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]];
    
    NSLog(@"Number of lines to save: %d", lines.count);
    if([withoutWhiteSpace length]<2)
    {
        NSLog(@"Error: saveCustomText - text too short");
        return NO;
    }
